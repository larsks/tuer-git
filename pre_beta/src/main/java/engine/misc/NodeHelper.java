    ...
    public static final Mesh makeCopy(Mesh mesh, boolean shareGeometricData) {
        ...
        if (mesh.getControllerCount() > 0) {
            ...
            if (controller instanceof KeyframeController) {
                ...
                // removes the controller in order to avoid it both in the
                // original mesh and in its copy
        ...
        if (keyframeController != null) {// makes a copy of the controller
            ...
            // puts back the controller into the original mesh (it mustn't be
            // done earlier to avoid using the same controller in the morph
            // mesh)
