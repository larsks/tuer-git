...
/**
 * Adaptation of the class FirstPersonControl in order to handle QWERTY (WSAD) &
 * AZERTY (ZSQD) keyboards correctly.
 * 
...
public final class ExtendedFirstPersonControl {
    ...
    protected void move(final Camera camera, final KeyboardState kb, final double tpf) {
        ...
        if ((rotX != 0 || rotY != 0) && mouseRotateSpeed != 0 && keyRotateSpeed != 0)
            rotate(camera, rotX * (keyRotateSpeed / mouseRotateSpeed) * tpf,
                    rotY * (keyRotateSpeed / mouseRotateSpeed) * tpf);
    }
    ...
    protected void onDragAction(final Canvas source, final TwoInputStates inputStates, final double tpf) {
        ...
        if (mouse.getDx() != 0 || mouse.getDy() != 0) {
            rotate(source.getCanvasRenderer().getCamera(), -mouse.getDx(),
                    lookUpDownReversed ? mouse.getDy() : -mouse.getDy());
        }
    }
    ...
    private static final class FirstPersonControlKeyboardTriggersPredicate implements Predicate<TwoInputStates> {
        ...
        private FirstPersonControlKeyboardTriggersPredicate(final ActionMap customActionMap) {
            inputs = customActionMap.getInputs(Action.MOVE_FORWARD, Action.MOVE_BACKWARD, Action.LOOK_UP,
                    Action.LOOK_DOWN, Action.STRAFE_LEFT, Action.STRAFE_RIGHT, Action.TURN_LEFT, Action.TURN_RIGHT);
        }
