...
public class UniformlyVariableMovementEquation implements MovementEquation {
    ...
    public UniformlyVariableMovementEquation(final double initialAcceleration, final double initialSpeed,
    ...
    /**
     * Replaces infinite and "not a number" number by a valid finite number
     * 
     * @param number
     *            ordinary number
     ...
     * 
     * @param value
     *            ordinary value (can be infinite or NaN)
    ...
    public final double getValueAtTime(final double elapsedTime) {
        return (validateNumber(validateValue((initialAcceleration / 2) * (elapsedTime * elapsedTime)
                + (elapsedTime * initialSpeed) + initialValue)));
    }
