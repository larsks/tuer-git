 ...
 * manager and the viewer. It contains the entry point of the editor.
 * 
...
public final class MainWindow {
     ...
     * 
     * @param applicativeFrame
     *            frame that contains the whole GUI
     */
    public MainWindow(JFrame applicativeFrame) {
        ...
        applicativeFrame.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0, false), "quit");
        ...
        applicativeFrame.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                .put(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK, false), "save");
        ...
        applicativeFrame.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                .put(KeyStroke.getKeyStroke(KeyEvent.VK_F5, 0, false), "refresh");
    ...
    /**
     * leaves the application as cleanly as possible (attempt to save all
     * projects)
     * 
     * @param confirm
     *            true if the user has to be prompted before closing
     */
    private final void quit(boolean confirm) {
        final boolean quitEnabled = (projectManager == null || projectManager.isQuitEnabled())
                && (toolManager == null || toolManager.isQuitEnabled());
        if (quitEnabled) {
            final boolean doIt;
            if (confirm)
                doIt = JOptionPane.showConfirmDialog(applicativeFrame, "Exit JFPSM?", "Confirm Exit",
                        JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION;
            else
                doIt = true;
            if (doIt) {// saves all projects
                if (projectManager != null)
                    projectManager.saveCurrentWorkspace();
                // destroys the window
                applicativeFrame.dispose();
                // this call is necessary to leave the application when using
                // Java Webstart
                System.exit(0);
            }
        }
    ...
    public String getStackTrace(final Throwable throwable) {
        ...
        } catch (final IOException ioe) {// it will never happen as StringWriter
    ...
    public static final void main(final String[] args) {
        ...
        // launches a minimal GUI to be able to display a popup if something
        // goes wrong later
    ...
    // TODO move this method into a separate class in order to avoid mixing
    // scene services and file services
