/**
 * Copyright (c) 2006-2015 Julien Gouesse This program is free software; you can
    ...
    public DeallocationHelper(final boolean ignoreClassesAndFieldsHints){
                    ...
                    if(/* javaVendor.equals("Apple Inc.")|| */javaVendor.equals("Free Software Foundation, Inc."))
                        {final String byteBufferAsNonByteBufferByteBufferFieldName="bb";
                         final String[] byteBufferAsNonByteBufferClassnames=new String[]{"java.nio.CharViewBufferImpl",
                                "java.nio.DoubleViewBufferImpl","java.nio.FloatViewBufferImpl",
                                "java.nio.IntViewBufferImpl","java.nio.LongViewBufferImpl",
                        ...
                        if(javaVendor.contains("Apache"))
                            {final String byteBufferAsNonByteBufferByteBufferFieldName="byteBuffer";
                             final String[] byteBufferAsNonByteBufferClassnames=new String[]{
                                    "java.nio.CharToByteBufferAdapter","java.nio.DoubleToByteBufferAdapter",
                                    "java.nio.FloatToByteBufferAdapter","java.nio.IntToByteBufferAdapter",
                                    "java.nio.LongToByteBufferAdapter","java.nio.ShortToByteBufferAdapter"};
                             for(final String byteBufferAsNonByteBufferClassname:byteBufferAsNonByteBufferClassnames)
                                 attachmentOrByteBufferFieldNameMap.put(byteBufferAsNonByteBufferClassname,byteBufferAsNonByteBufferByteBufferFieldName);
                            }
                        else
            ...
            for(final Entry<String,String> attachmentOrByteBufferFieldNameEntry:attachmentOrByteBufferFieldNameMap.entrySet())
                {final String classname=attachmentOrByteBufferFieldNameEntry.getKey();
                 ...
                 try
                     ...
                     {// TODO The Java version isn't very useful under
                      // Android as it is always zero, rather use
                      // android.os.Build.VERSION.RELEASE to show something
                      // meaningful supported since the API level 1
        ...
        if(attachmentOrByteBufferFieldNameMap.isEmpty())
            {// detects everything with the reflection API
             // creates all possible kinds of direct NIO buffer that can contain
             // buffers (sliced buffers and views)
        ...
        if(buffer!=null&&buffer.isDirect())
            {// looks for any contained buffer in the passed buffer
             ...
             if(attachmentBufferOrByteBuffer==null)
                 {// if it's a direct byte buffer and if it's an instance of
                  // a deallocatable buffer class
                  if(buffer instanceof ByteBuffer&&deallocatableBufferClassSet.contains(bufferClass))
                      ...
                      {// it's not a byte buffer or it's not a
                 ...
                 {// the passed buffer contains another buffer, looks for a
