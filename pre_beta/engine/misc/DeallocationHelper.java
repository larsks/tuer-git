...
/**
 * Helper to deallocate memory on the native heap allocated during the creation of a direct byte buffer. It supports numerous
 * virtual machines including OpenJDK, Oracle/Sun Java, Android Dalvik Virtual Machine, Apache Harmony and GNU Classpath. This 
 * class uses the syntax of Java 1.7 but it can work correctly with Java 1.4 with a very few minor type changes when using the 
 * maps and the collections. It relies on lots of implementation details but it's robust enough to go on working (except when 
 * the implementors intentionally use a very general class to store the buffers) despite minor naming changes like those that 
 * occurred between Java 1.6 and Java 1.7. It supports Java 1.9 (see the JEP 260) as the only internal class in use is 
 * sun.misc.Cleaner.
 * 
 * N.B: Releasing the native memory of a sliced direct NIO buffer, the one of a direct NIO buffer created with JNI or the one 
 * of any direct NIO buffer created by the virtual machine or by a framework not under your control doesn't prevent the calls 
 * to methods attempting to access such buffers. Those calls can throw an exception or crash the virtual machine depending on 
 * the implementations.
 * 
...
public class DeallocationHelper{
	...
	public static abstract class Deallocator{
		 ...
		 */
		public abstract void run(final ByteBuffer directByteBuffer);
	}
	...
	public static class OracleSunOpenJdkDeallocator extends Deallocator{
		...
		@Override
		public void run(final ByteBuffer directByteBuffer){
	 ...
	 * Main constructor
	 * 
	 * @param ignoreClassesAndFieldsHints <code>true</code> if the known implementation details should be ignored when looking for the 
	 * classes and the fields used for the native memory of the direct buffers (they are then fully recomputed at runtime which is slower 
	 * but safer), otherwise <code>false</code>
	 */
	public DeallocationHelper(final boolean ignoreClassesAndFieldsHints){
		super();
		final List<Buffer> buffersToDelete=new ArrayList<>();
		/**
		 * builds the map used to determine the names of the fields containing the direct byte buffers. The direct read only buffers and
		 * the sliced buffers and the direct buffers for other primitive types than bytes store their data into some direct byte buffers. 
		 * Those direct byte buffers often are the only one accessing directly to the native memory. That's why it's necessary to find 
		 * them when a developer passes a direct NIO buffer. The code below relies on numerous implementation details found in some classes 
		 * not available in the public APIs, it's used to find the fields faster in most of the cases. The class names haven't changed 
		 * since Java 1.4 unlike a few field names.
		 */
		...
		if(!ignoreClassesAndFieldsHints)
		    {if(javaVendor.equals("Sun Microsystems Inc.")||javaVendor.equals("Oracle Corporation"))
		         {final String java14to16DirectBufferAttachmentFieldName = "viewedBuffer";
	              ...
	              final String[] directBufferClassnames=new String[]{
	              "java.nio.DirectByteBuffer","java.nio.DirectByteBufferR",
	              "java.nio.DirectCharBufferRS","java.nio.DirectCharBufferRU","java.nio.DirectCharBufferS","java.nio.DirectCharBufferU",
	              "java.nio.DirectDoubleBufferRS","java.nio.DirectDoubleBufferRU","java.nio.DirectDoubleBufferS","java.nio.DirectDoubleBufferU",
	              "java.nio.DirectFloatBufferRS","java.nio.DirectFloatBufferRU","java.nio.DirectFloatBufferS","java.nio.DirectFloatBufferU",
	              "java.nio.DirectIntBufferRS","java.nio.DirectIntBufferRU","java.nio.DirectIntBufferS","java.nio.DirectIntBufferU",
	              "java.nio.DirectLongBufferRS","java.nio.DirectLongBufferRU","java.nio.DirectLongBufferS","java.nio.DirectLongBufferU",
	              "java.nio.DirectShortBufferRS","java.nio.DirectShortBufferRU","java.nio.DirectShortBufferS","java.nio.DirectShortBufferU"};
	              final String[] byteBufferAsNonByteBufferClassnames=new String[]{
	              "java.nio.ByteBufferAsCharBufferB","java.nio.ByteBufferAsCharBufferL","java.nio.ByteBufferAsCharBufferRB","java.nio.ByteBufferAsCharBufferRL",
	              "java.nio.ByteBufferAsDoubleBufferB","java.nio.ByteBufferAsDoubleBufferL","java.nio.ByteBufferAsDoubleBufferRB","java.nio.ByteBufferAsDoubleBufferRL",
	              "java.nio.ByteBufferAsFloatBufferB","java.nio.ByteBufferAsFloatBufferL","java.nio.ByteBufferAsFloatBufferRB","java.nio.ByteBufferAsFloatBufferRL",
	              "java.nio.ByteBufferAsIntBufferB","java.nio.ByteBufferAsIntBufferL","java.nio.ByteBufferAsIntBufferRB","java.nio.ByteBufferAsIntBufferRL",
	              "java.nio.ByteBufferAsLongBufferB","java.nio.ByteBufferAsLongBufferL","java.nio.ByteBufferAsLongBufferRB","java.nio.ByteBufferAsLongBufferRL",
	              "java.nio.ByteBufferAsShortBufferB","java.nio.ByteBufferAsShortBufferL","java.nio.ByteBufferAsShortBufferRB","java.nio.ByteBufferAsShortBufferRL"};
			     ...
			     if(javaVendor.equals("The Android Project"))
			         {final String byteBufferAsNonByteBufferByteBufferFieldName="byteBuffer";
			          final String[] byteBufferAsNonByteBufferClassnames=new String[]{
			          "java.nio.ByteBufferAsCharBuffer","java.nio.ByteBufferAsDoubleBuffer","java.nio.ByteBufferAsFloatBuffer", 
			          "java.nio.ByteBufferAsIntBuffer","java.nio.ByteBufferAsLongBuffer","java.nio.ByteBufferAsShortBuffer"};
			          for(final String byteBufferAsNonByteBufferClassname:byteBufferAsNonByteBufferClassnames)
		                  attachmentOrByteBufferFieldNameMap.put(byteBufferAsNonByteBufferClassname,byteBufferAsNonByteBufferByteBufferFieldName);
			         }
			     else
				     if(/*javaVendor.equals("Apple Inc.")||*/javaVendor.equals("Free Software Foundation, Inc."))
				         {final String byteBufferAsNonByteBufferByteBufferFieldName="bb";
				          final String[] byteBufferAsNonByteBufferClassnames=new String[]{
					      "java.nio.CharViewBufferImpl","java.nio.DoubleViewBufferImpl","java.nio.FloatViewBufferImpl", 
					      "java.nio.IntViewBufferImpl","java.nio.LongViewBufferImpl","java.nio.ShortViewBufferImpl"};
				          for(final String byteBufferAsNonByteBufferClassname:byteBufferAsNonByteBufferClassnames)
			                  attachmentOrByteBufferFieldNameMap.put(byteBufferAsNonByteBufferClassname,byteBufferAsNonByteBufferByteBufferFieldName);
				         }
				     else
					     if(javaVendor.contains("Apache"))
				             {final String byteBufferAsNonByteBufferByteBufferFieldName="byteBuffer";
				              final String[] byteBufferAsNonByteBufferClassnames=new String[]{
						      "java.nio.CharToByteBufferAdapter","java.nio.DoubleToByteBufferAdapter","java.nio.FloatToByteBufferAdapter", 
						      "java.nio.IntToByteBufferAdapter","java.nio.LongToByteBufferAdapter","java.nio.ShortToByteBufferAdapter"};
						      for(final String byteBufferAsNonByteBufferClassname:byteBufferAsNonByteBufferClassnames)
					              attachmentOrByteBufferFieldNameMap.put(byteBufferAsNonByteBufferClassname,byteBufferAsNonByteBufferByteBufferFieldName);
				             }
					     else
						     if(javaVendor.equals("Jeroen Frijters"))
						         {//TODO IKVM
						         }
						     else
			          ...
			          if(bufferField==null)
			              {final String superClassesMsg;
			        	   if(intermediaryClassWithoutBufferList.isEmpty())
			        		   ...
			        		   if(intermediaryClassWithoutBufferList.size()==1)
			    	      ...
			    	      {//the field has been found, stores it into the map
			    	       attachmentOrByteBufferFieldMap.put(bufferClass,bufferField);
			    	      }
			         }
			     catch(ClassNotFoundException cnfe)
			     {//TODO The Java version isn't very useful under Android as it is always zero, rather use android.os.Build.VERSION.RELEASE to show something meaningful supported since the API level 1
		...
		//if a known implementation has drastically changed or if the current implementation is unknown
		if(attachmentOrByteBufferFieldNameMap.isEmpty())
		...
		if(deallocatableBufferClassSet.isEmpty())
		    {//creates a read write direct byte buffer
		...
		//builds the deallocator responsible for releasing the native memory of a deallocatable byte buffer
		if(javaVendor.equals("Sun Microsystems Inc.")||javaVendor.equals("Oracle Corporation"))
	...
	public ByteBuffer findDeallocatableBuffer(Buffer buffer) {
        ...
        if(buffer!=null&&buffer.isDirect())
            {//looks for any contained buffer in the passed buffer
        	 final Class<?> bufferClass=buffer.getClass();
             ...
             if(attachmentBufferOrByteBuffer==null)
                 {//if it's a direct byte buffer and if it's an instance of a deallocatable buffer class
            	  if(buffer instanceof ByteBuffer&&deallocatableBufferClassSet.contains(bufferClass))
                      ...
                      {//it's not a byte buffer or it's not a deallocatable buffer
                 ...
                 {//the passed buffer contains another buffer, looks for a deallocatable buffer inside it
            	  deallocatableDirectByteBuffer=findDeallocatableBuffer(attachmentBufferOrByteBuffer);
                 }
            }
        else
            {//there is no need to clean the heap based buffers
        	 deallocatableDirectByteBuffer=null;
            }
	...
	/*public static void main(String[] args){
		final ByteBuffer bigEndianReadOnlyDirectByteBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.BIG_ENDIAN).asReadOnlyBuffer();
		...
		final ShortBuffer bigEndianReadOnlyDirectShortBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.BIG_ENDIAN).asReadOnlyBuffer().asShortBuffer();
		final ShortBuffer bigEndianReadWriteDirectShortBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.BIG_ENDIAN).asShortBuffer();
		final ByteBuffer littleEndianReadOnlyDirectByteBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.LITTLE_ENDIAN).asReadOnlyBuffer();
		final ByteBuffer littleEndianReadWriteDirectByteBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.LITTLE_ENDIAN);
		final ByteBuffer slicedLittleEndianReadOnlyDirectByteBuffer=((ByteBuffer)ByteBuffer.allocateDirect(2).order(ByteOrder.LITTLE_ENDIAN).put((byte)0).put((byte)0).position(1).limit(2)).slice().asReadOnlyBuffer();
		final ByteBuffer slicedLittleEndianReadWriteDirectByteBuffer=((ByteBuffer)ByteBuffer.allocateDirect(2).order(ByteOrder.LITTLE_ENDIAN).put((byte)0).put((byte)0).position(1).limit(2)).slice();
		final CharBuffer littleEndianReadOnlyDirectCharBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.LITTLE_ENDIAN).asReadOnlyBuffer().asCharBuffer();
		final CharBuffer littleEndianReadWriteDirectCharBuffer=ByteBuffer.allocateDirect(1).order(ByteOrder.LITTLE_ENDIAN).asCharBuffer();
