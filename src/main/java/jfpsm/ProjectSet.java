...
public final class ProjectSet extends JFPSMProjectUserObject {
     ...
     * 
     * @param projectName
     *            name of the project whose file is searched
     * @return project file
     */
    ...
    final void saveProject(Project project) {
        ...
        if (projectFile == null) {// creates a new file with the default
                                  // filename
    ...
    final void saveProject(Project project, final File file) {
        if (projectsList.contains(project)) {// this should be tested rather in
                                             // the GUI
                                             // checks if the internal project
                                             // has unsaved modifications or
                                             // if it is an external (exported)
                                             // project
            final File parentFile = file.getParentFile();
            if (project.isDirty() || parentFile == null || !parentFile.equals(workspaceDirectory)) {
                File tmpFile = null;
                // saves the project (project.xml and image files have to be put
                // into a ZIP file)
                try {
                    if (!file.exists()) {
                    ...
                    // creates a temporary file used as a buffer before putting
                    // the data into the archive
                    tmpFile = File.createTempFile("JFPSM", ".tmp");
                    try (ZipOutputStream zoStream = new ZipOutputStream(
                        ...
                        /**
                         * Actually, it "should" be possible to use the existing
                         * ZIP output stream to avoid creating a temporary file
                         * by flushing the encoder to avoid closing this stream
                         * but it throws a SAX exception. It is necessary to
                         * close the XML node by doing zoStream.write(
                         * "</java> \n".getBytes());
                         */
                        try (CustomXMLEncoder encoder = new CustomXMLEncoder(
    ...
    private final void initializeWorkspaceDirectory() {
        ...
        if (!workspaceDirectory.exists() || !workspaceDirectory.isDirectory() || !workspaceDirectory.canRead()
                || !workspaceDirectory.canWrite())
            throw new RuntimeException(
                    "The workspace directory " + workspaceDirectory.getAbsolutePath() + " cannot be used!");
    }
    ...
    /**
     * Gets the names of the projects in the file system. It may contain
     * duplicates
     * 
     ...
     * @param projectFile
     *            project file
     * @return the newly loaded project or the previous one if it had been
     *         already loaded
     */
    final Project loadProject(File projectFile) {
        ...
        if (projectFile.getName().endsWith(Project.getFileExtension())) {
            ...
            try (ZipFile zipFile = new ZipFile(projectFile)) {
                ZipEntry entry;
                // at first, gets the file project.xml to build the project
                    ...
                    if (decodedObject != null && decodedObject instanceof Project) {
                        ...
                        while (entries.hasMoreElements()) {
                            ...
                            if (!entry.getName().equals("project.xml")) {
                                if (!entry.isDirectory()) {
                                    ...
                                    if (path.length == 4 && path[0].equals("levelset")) {// finds
                                                                                         // the
                                                                                         // floor
                                                                                         // that
                                                                                         // should
                                        ...
                                        for (FloorSet floorSet : project.getLevelSet().getFloorSetsList())
                                            for (Floor floor : floorSet.getFloorsList())
                                                if (path[1].equals(floorSet.getName())
                                                        && path[2].equals(floor.getName())) {
                                    ...
                                    } else if (path.length == 2 && path[0].equals("tileset")) {// find
                                                                                               // the
                                                                                               // tile
                                                                                               // that
                                                                                               // should
                                        ...
                                        for (Tile tile : project.getTileSet().getTilesList())
                                            if (path[1].startsWith(tile.getName()) && path[1].endsWith(".png")) {
                                                textureIndex = -1;
                                                textureIndexString = path[1].substring(tile.getName().length(),
                                                        path[1].lastIndexOf(".png"));
                                                try {
                        ...
                        if (!addProject(project)) {
                            for (Project existingProject : projectsList)
                                if (existingProject.getName().equals(projectName)) {// FIXME:
                                                                                    // handle
                                                                                    // conflicts
                        ...
                        throw new IllegalArgumentException(
                                "The file named \"project.xml\" does not contain a valid project!");
                } else
                    throw new IllegalArgumentException(
                            "The project file " + fullname + " does not contain any file named \"project.xml\"!");
