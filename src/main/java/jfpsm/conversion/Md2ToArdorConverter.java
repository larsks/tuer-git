...
public final class Md2ToArdorConverter {
    ...
    public static final void main(String[] args) throws IOException, URISyntaxException {
        ...
        try {
            SimpleResourceLocator srl = new SimpleResourceLocator(Md2ToArdorConverter.class.getResource("/images"));
        ...
        for (String arg : args) {
            ...
            destFile = new File(
                    sourceFile.getAbsolutePath().substring(0, sourceFile.getAbsolutePath().lastIndexOf(".md2"))
                            + ".abin");
            if (!destFile.exists())
            ...
            imageSourceFile = new File(
                    sourceFile.getName().substring(0, sourceFile.getName().lastIndexOf(".md2")) + ".png");
            imageSource = (URLResourceSource) ResourceLocatorTool.locateResource(ResourceLocatorTool.TYPE_TEXTURE,
                    imageSourceFile.getName());
            if (imageSource != null) {
                ts = new TextureState();
                ts.setEnabled(true);
                ts.setTexture(TextureManager.load(new URLResourceSource(imageSource.getURL()),
