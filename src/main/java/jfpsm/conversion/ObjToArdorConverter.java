...
/**
 * Converter of WaveFront OBJ files into Ardor3D binary files. As the WaveFront
 * OBJ format doesn't support vertex color, it is optionally possible to use the
 * ambient colors of the materials as the vertices colors
 * 
...
public class ObjToArdorConverter {
    ...
    /**
     * flag indicating whether to convert ambient colors (of materials, when the
     * lighting is enabled) into vertices colors (with no material, when the
     * lighting is disabled)
     */
    private boolean conversionOfAmbientColorsIntoVerticesColorsEnabled;

    /**
     * flag indicating whether to convert ambient colors into texture
     * coordinates
     */
    ...
    public void setConversionOfAmbientColorsIntoVerticesColorsEnabled(
            final boolean conversionOfAmbientColorsIntoVerticesColorsEnabled) {
    ...
    public void run(final String[] args) throws IOException, URISyntaxException {
        ...
        BinaryExporter binaryExporter = conversionOfAmbientColorsIntoVerticesColorsEnabled
                || conversionOfAmbientColorsIntoTexCoordsEnabled ? new DirectBinaryExporter() : new BinaryExporter();
        try {
            SimpleResourceLocator srl = new SimpleResourceLocator(ObjToArdorConverter.class.getResource("/images"));
        ...
        try {
            objImporter.setTextureLocator(new SimpleResourceLocator(ObjToArdorConverter.class.getResource("/images")));
        ...
        for (String arg : args) {
            ...
            File destFile = new File(
                    sourceFile.getAbsolutePath().substring(0, sourceFile.getAbsolutePath().lastIndexOf(".obj"))
                            + ".abin");
            if (!destFile.exists())
            ...
            if (spatial instanceof Mesh) {
                ...
                if (materialState != null) {
                    ...
                    if (ambientRgbaColor != null) {
                        ...
                        if (meshData != null && meshData.getTextureCoords(0) == null) {// gets
                                                                                       // the
                                                                                       // vertex
                                                                                       // buffer
                            ...
                            if (vertexBuffer != null) {
                                ...
                                // creates the new texture coordinate buffer
                                // data of the mesh
                                ...
                                final Vector3[] triangleVertices = new Vector3[] { new Vector3(), new Vector3(),
                                        new Vector3() };
                                final Vector2[] triangleTexCoords = new Vector2[] { new Vector2(), new Vector2(),
                                        new Vector2() };
                                ...
                                for (int sectionIndex = 0, sectionCount = meshData
                                    ...
                                    // only takes care of sections containing
                                    // triangles
                                    if (meshData.getIndexMode(sectionIndex) == IndexMode.Triangles) {// loops
                                                                                                     // on
                                                                                                     // all
                                                                                                     // triangles
                                                                                                     // of
                                        ...
                                        for (int trianglePrimitiveIndex = 0, triangleCount = meshData.getPrimitiveCount(
                                                sectionIndex); trianglePrimitiveIndex < triangleCount; trianglePrimitiveIndex++) {// gets
                                                                                                                                  // the
                                                                                                                                  // 3
                                                                                                                                  // vertices
                                                                                                                                  // of
                                                                                                                                  // the
                                                                                                                                  // triangle
                                            meshData.getPrimitiveVertices(trianglePrimitiveIndex, sectionIndex,
                                                    triangleVertices);
                                            // TODO use a better algorithm to
                                            ...
                                            // puts the texture coordinates into
                                            // the buffer
                                            for (Vector2 triTexCoord : triangleTexCoords) {
                                ...
                                // sets the new texture coordinate buffer data
                                // of the mesh
    ...
    public static final void main(final String[] args) {
        try {
            final ObjToArdorConverter converter = new ObjToArdorConverter();
