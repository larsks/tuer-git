...
public class DirectedRootedTree<V, E> extends DirectedAcyclicGraph<V, E> {
        ...
        if (result) {
            if (getVertexCount() > 0) {
                ...
                if (getEdgeCount() > 0) {/**
                                          ...
                                          * the second vertex is already an
                                          * ancestor of the first vertex as such
                                          * a loop would be detected in the
                                          * super class
                                          */
                    if (containsVertex(secondVertex))
                        if (containsVertex(firstVertex)) {// the new edge would
                                                          // create a loop
                            result = false;
                        } else {
                            final int incomingEdgesCount = internalGetIncomingEdges(secondVertex).size();
                            /**
                             * the vertices (except the root) must have exactly
                             * one incoming vertex after the addition, i.e this
                             * addition is possible only if the second vertex is
                             * the root
                             */
                            result = incomingEdgesCount == 0;
                            /**
                             * if the addition is successful, the first vertex
                             * will become the root
                             */
                        }
                    else if (containsVertex(
                            firstVertex)) {/**
                                            * the new edge is connected to an
                                            * existing vertex
                                            */
                        result = true;
                    } else {
                ...
                } else {// there is no edge, there is only the root
                    final V vertex = this.vertices.keySet().iterator().next();
                    result = firstVertex.equals(vertex) || secondVertex.equals(vertex);
                    /**
                     * if the current first vertex is not yet the root, it will
                     * become the root
                     */
                }
            } else {/**
    ...
    public boolean addVertex(V vertex) {
        ...
        if (vertices.isEmpty()) {// tries to add this vertex as the graph is
                                 // empty
            ...
            if (success)
                 ...
                 * there is already at least one vertex, another disconnected
                 * one cannot be added
                 */
            success = false;
        }
    ...
    public boolean removeVertex(V vertex) {
        ...
        if (success) {
            ...
            if (!incomingEdges.isEmpty()) {// this vertex is not the root
                /**
                 * uses a visitor (BFS) to get its direct and indirect
                 * successors
                 */
                final DirectedConnectedComponentVisitorWithHistory<V, E, DirectedRootedTree<V, E>> visitor = new DirectedConnectedComponentVisitorWithHistory<>();
                ...
                /**
                 * starts using this set by the end, reverses the order of the
                 * vertices
                 */
                Collections.reverse(directAndIndirectsuccessors);
                // removes all its successors and so on...
                for (V vertexToRemove : directAndIndirectsuccessors) {// gets
                                                                      // the
                                                                      // incoming
                                                                      // edges
                                                                      // (there
            ...
            } else {// this vertex is the root, the tree must be emptied
                edges.clear();
                vertices.clear();
                root = null;
            }
