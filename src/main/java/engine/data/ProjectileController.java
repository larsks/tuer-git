...
/**
 * Controller of a projectile which makes a uniformly accelerated rectilinear
 * motion.
 * 
...
public class ProjectileController implements SpatialController<Node> {
    ...
    public void update(double timeSinceLastCall, Node caller) {
        ...
        final long elapsedTimeSinceLatestCallInNanos = absoluteElapsedTimeInNanoseconds
                - projectileData.getInitialTimeInNanos();
        // computes the length of the whole move since the very beginning
        final double translationLength = ((projectileData.getInitialAcceleration() / 2)
                * elapsedTimeSinceLatestCallInNanos * elapsedTimeSinceLatestCallInNanos)
