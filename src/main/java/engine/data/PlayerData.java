...
public class PlayerData {
    ...
    /**
     * flag indicating whether the player is using 2 identical weapons
     * simultaneously
     */
    ...
    /**
     * weapon container of the right hand (right-handed) or in the left hand
     * (left-handed)
     */
    private final WeaponContainer primaryHandWeaponContainer;
    /**
     * weapon container of the left hand (right-handed) or in the right hand
     * (left-handed)
     */
    ...
    public PlayerData(final CameraNode cameraNode, final AmmunitionFactory ammunitionFactory,
            final WeaponFactory weaponFactory, final Map<Ammunition, Integer> ammunitionMaxCountMap,
    ...
    public int collect(final Node collectible) {
        ...
        if (userData != null && userData instanceof CollectibleUserData) {
            ...
            else if (userData instanceof MedikitUserData)
                result = collectMedikit(collectible, (MedikitUserData) userData);
            else if (userData instanceof AmmunitionBoxUserData)
    ...
    protected int collectWeapon(final Node collectible, final WeaponUserData weaponUserData) {
        ...
        if ((ownerUid == uid || canChangeOfOwner)) {
            if (weaponUserData.isPrimary())
                ...
                // the weapon must be already in the primary container to be
                // added into the second one
                result = primaryHandWeaponContainer.isAvailable(weapon)
                        && secondaryHandWeaponContainer.add(collectible, weapon);
    ...
    // if(!isAlive())
    // {/**
    // * TODO change the owner uid (if there is no digital watermark) and detach
     ...
     * 
     * @return a value in the interval [0;1] representing the progress of the
     *         "put back" according to the current ordinate of the weapon
     */
     ...
     * 
     * @return a value in the interval [0;1] representing the progress of the
     *         attack ending according to the current ordinate of the weapon
     */
    public double computeEndAttackProgress() {
        ...
        if (isCurrentWeaponAmmunitionCountDisplayable()) {
            ...
            if (y == endAttackYStart)
                endAttackProgress = 0;
            else if (y == endAttackYEnd)
                endAttackProgress = 1;
            else
                endAttackProgress = Math.max(0,
                        Math.min(1.0d, (y - endAttackYStart) / (endAttackYEnd - endAttackYStart)));
        } else
     ...
     * 
     * @param elapsedTimeSincePutBackStartInSeconds
     *            elapsed time since the start of the "put back" step expressed
     *            in seconds
     * @param initialPutBackProgress
     *            initial progress of the "put back"
     */
    public void putBack(final double elapsedTimeSincePutBackStartInSeconds, final double initialPutBackProgress) {
        if (isCurrentWeaponAmmunitionCountDisplayable()) {// computes the
                                                          // progress of the
                                                          // "put back" step (in
                                                          // the interval [0;1])
            final double putBackStepProgress = Math.max(0, Math.min(1.0d,
                    initialPutBackProgress + (elapsedTimeSincePutBackStartInSeconds / PUT_BACK_DURATION_IN_SECONDS)));
            final double putBackYStart = PULLED_OUT_WEAPON_ORDINATE;
            final double putBackYEnd = PUT_BACK_WEAPON_ORDINATE;
            ...
            primaryWeaponNode.setTranslation(primaryWeaponNode.getTranslation().getX(), putBackYCurrent,
                    primaryWeaponNode.getTranslation().getZ());
            if (isDualWeaponUseEnabled()) {
                ...
                secondaryWeaponNode.setTranslation(secondaryWeaponNode.getTranslation().getX(), putBackYCurrent,
                        secondaryWeaponNode.getTranslation().getZ());
            }
        }
    }
     ...
     * 
     * @param elapsedTimeSincePullOutStartInSeconds
     *            elapsed time since the start of the "pull out" step expressed
     *            in seconds
     */
    public void pullOut(final double elapsedTimeSincePullOutStartInSeconds) {
        if (isCurrentWeaponAmmunitionCountDisplayable()) {// computes the
                                                          ...
                                                          // "pull out" step (in
                                                          // the interval [0;1])
            final double pullOutStepProgress = Math.max(0,
                    Math.min(1.0d, elapsedTimeSincePullOutStartInSeconds / PULL_OUT_DURATION_IN_SECONDS));
            ...
            else if (pullOutStepProgress == 1)
                pullOutYCurrent = pullOutYEnd;
            else
                pullOutYCurrent = pullOutYStart + ((pullOutYEnd - pullOutYStart) * pullOutStepProgress);
            final Node primaryWeaponNode = primaryHandWeaponContainer.getNode(weaponInUse);
            // modifies the ordinate of the primary weapon
            primaryWeaponNode.setTranslation(primaryWeaponNode.getTranslation().getX(), pullOutYCurrent,
                    primaryWeaponNode.getTranslation().getZ());
            if (isDualWeaponUseEnabled()) {
                final Node secondaryWeaponNode = secondaryHandWeaponContainer.getNode(weaponInUse);
                // modifies the ordinate of the secondary weapon
                secondaryWeaponNode.setTranslation(secondaryWeaponNode.getTranslation().getX(), pullOutYCurrent,
                        secondaryWeaponNode.getTranslation().getZ());
            }
        }
    }
     ...
     * 
     * @return <code>true</code> if the "put back" is complete, otherwise
     *         <code>false</code>
     */
     ...
     * 
     * @return <code>true</code> if the "pull out" is complete, otherwise
     *         <code>false</code>
     */
    ...
    public void attack(final double elapsedTimeSinceAttackStartInSeconds) {
        ...
        final double attackStepProgress = Math.max(0,
                Math.min(1.0, (elapsedTimeSinceAttackStartInSeconds / currentWeaponBlowOrShotDurationInSeconds) % 1.0));
        if (isCurrentWeaponAmmunitionCountDisplayable()) {// TODO sparks?
            /*
            ...
            if (attackYCurrent < PULLED_OUT_WEAPON_ORDINATE)
                System.out
                        .println("pb: " + attackYCurrent + " attackVariationPercentage: " + attackVariationPercentage);
            final Node primaryWeaponNode = primaryHandWeaponContainer.getNode(weaponInUse);
            // modifies the ordinate of the primary weapon
            primaryWeaponNode.setTranslation(primaryWeaponNode.getTranslation().getX(), attackYCurrent,
                    primaryWeaponNode.getTranslation().getZ());
            if (isDualWeaponUseEnabled()) {
                final Node secondaryWeaponNode = secondaryHandWeaponContainer.getNode(weaponInUse);
                // modifies the ordinate of the secondary weapon
                secondaryWeaponNode.setTranslation(secondaryWeaponNode.getTranslation().getX(), attackYCurrent,
                        secondaryWeaponNode.getTranslation().getZ());
            }
        } else {
    ...
    public void waitForAttackEnd(final double elapsedTimeSinceWaitForAttackEndStartInSeconds,
            final double initialEndAttackProgress) {
        if (isCurrentWeaponAmmunitionCountDisplayable()) {// computes the
                                                          // progress of the
                                                          // "put back" step (in
                                                          // the interval [0;1])
            final double endAttackStepProgress = Math.max(0, Math.min(1.0d, initialEndAttackProgress
                    + (elapsedTimeSinceWaitForAttackEndStartInSeconds / ATTACK_END_DURATION_IN_SECONDS)));
            final double endAttackYStart = ATTACK_WEAPON_MAXIMUM_ORDINATE;
            final double endAttackYEnd = PULLED_OUT_WEAPON_ORDINATE;
            // computes the ordinate with the progress
            ...
            else if (endAttackStepProgress == 1)
                endAttackYCurrent = endAttackYEnd;
            else
                endAttackYCurrent = endAttackYStart + ((endAttackYEnd - endAttackYStart) * endAttackStepProgress);
            final Node primaryWeaponNode = primaryHandWeaponContainer.getNode(weaponInUse);
            // modifies the ordinate of the primary weapon
            primaryWeaponNode.setTranslation(primaryWeaponNode.getTranslation().getX(), endAttackYCurrent,
                    primaryWeaponNode.getTranslation().getZ());
            if (isDualWeaponUseEnabled()) {
                final Node secondaryWeaponNode = secondaryHandWeaponContainer.getNode(weaponInUse);
                // modifies the ordinate of the secondary weapon
                secondaryWeaponNode.setTranslation(secondaryWeaponNode.getTranslation().getX(), endAttackYCurrent,
                        secondaryWeaponNode.getTranslation().getZ());
            }
        }
    }
    ...
    /**
     * Gets the amount of ammo that can be used during a reload of the weapon in
     * the primary hand
     * 
     * @return amount of ammo that can be used during a reload of the weapon in
     *         the primary hand
     */
    public int getReloadableAmmoCountForPrimaryHandWeapon() {
        final int reloadableAmmoCount;
        if (weaponInUse != null && !weaponInUse.isForMelee()) {// gets the size
                                                               // of the
                                                               // magazine of
                                                               // the weapon
                                                               // currently in
            ...
            final WeaponUserData primaryHandWeaponUserData = (WeaponUserData) primaryHandWeaponContainer
                    .getNode(weaponInUse).getUserData();
            // computes the remaining room for ammo in the magazine
            final int remainingRoomForAmmoInMagazineForPrimaryHandWeapon = magazineSize
                    - primaryHandWeaponUserData.getAmmunitionCountInMagazine();
            ...
            // computes the amount of available ammo in the container if the
            // player reloads the weapon currently in use
            final int availableAmmoInContainerAfterReload = Math.max(0,
                    availableAmmoInContainerBeforeReload - remainingRoomForAmmoInMagazineForPrimaryHandWeapon);
    ...
    public int getReloadableAmmoCountForSecondaryHandWeapon() {
        final int reloadableAmmoCount;
        if (weaponInUse != null && !weaponInUse.isForMelee() && dualWeaponUseEnabled) {// gets
                                                                                       // the
                                                                                       // size
                                                                                       // of
                                                                                       // the
            ...
            final int magazineSize = weaponInUse.getMagazineSize();
            // gets the data about the weapon currently in use
            final WeaponUserData secondaryHandWeaponUserData = (WeaponUserData) secondaryHandWeaponContainer
                    .getNode(weaponInUse).getUserData();
            // computes the remaining room for ammo in the magazine
            final int remainingRoomForAmmoInMagazineForSecondaryHandWeapon = magazineSize
                    - secondaryHandWeaponUserData.getAmmunitionCountInMagazine();
            // gets the amount of available ammo in the container
            final int availableAmmoInContainerBeforeReload = getAmmunitionCountInContainer();
            // computes the amount of available ammo in the container if the
            // player reloads the weapon currently in use
            final int availableAmmoInContainerAfterReload = Math.max(0,
                    availableAmmoInContainerBeforeReload - remainingRoomForAmmoInMagazineForSecondaryHandWeapon);
            // computes the amount of available ammo for a single reload
            reloadableAmmoCount = availableAmmoInContainerBeforeReload - availableAmmoInContainerAfterReload;
        } else
            reloadableAmmoCount = 0;
    ...
    public int reload() {
        ...
        if (weaponInUse != null && !weaponInUse.isForMelee()) {// gets the ammo
                                                               // type
            final Ammunition ammo = weaponInUse.getAmmunition();
            // gets the data about the weapon currently in use
            final WeaponUserData primaryHandWeaponUserData = (WeaponUserData) primaryHandWeaponContainer
                    .getNode(weaponInUse).getUserData();
            // gets the amount of ammo that can really be used during a reload
            // (depending on the container and the magazine)
            ...
            final int availableAmmoForPrimaryHandWeaponReload = ammoContainerContainer.remove(ammo,
                    reloadableAmmoCountForPrimaryHandWeapon);
            ...
            if (dualWeaponUseEnabled) {
                ...
                // removes ammo from the container
                final int availableAmmoForSecondaryHandWeaponReload = ammoContainerContainer.remove(ammo,
                        reloadableAmmoCountForSecondaryHandWeapon);
                // adds it into the magazine
     ...
     * Launches an attack
     * 
     * @return consumed ammunition if the weapon is not a melee weapon, knock
     *         count otherwise
     */
    public Map.Entry<Integer, Integer> attack() {
        ...
        if (weaponInUse != null && !weaponInUse.isForMelee()) {
            ...
            final WeaponUserData primaryHandWeaponUserData = (WeaponUserData) primaryHandWeaponContainer
                    .getNode(weaponInUse).getUserData();
            consumedAmmunitionOrKnockCountWithPrimaryHand = primaryHandWeaponUserData
                    .removeAmmunitionFromMagazine(ammoPerShot);
            if (dualWeaponUseEnabled) {
                final WeaponUserData secondaryHandWeaponUserData = (WeaponUserData) secondaryHandWeaponContainer
                        .getNode(weaponInUse).getUserData();
                consumedAmmunitionOrKnockCountWithSecondaryHand = secondaryHandWeaponUserData
                        .removeAmmunitionFromMagazine(ammoPerShot);
            } else
        ...
        final Map.Entry<Integer, Integer> consumedAmmunitionOrKnockCounts = new AbstractMap.SimpleEntry<>(
                Integer.valueOf(consumedAmmunitionOrKnockCountWithPrimaryHand),
                Integer.valueOf(consumedAmmunitionOrKnockCountWithSecondaryHand));
     ...
     * 
     * @return <code>true</code> if the player can attack with his current
     *         weapon, otherwise <code>false</code>
     */
    public boolean canAttack() {
        ...
        if (weaponInUse != null) {
            if (weaponInUse.isForMelee())
                ...
                canAttack = getAmmunitionCountInPrimaryHandedWeapon() > 0
                        || getAmmunitionCountInSecondaryHandedWeapon() > 0;
        } else
            canAttack = true;
        return (canAttack);
    }
     ...
     * 
     * @return <code>true</code> if the player can reload his current weapon,
     *         otherwise <code>false</code>
     */
    public boolean canReload() {
        final boolean canReload = getReloadableAmmoCountForPrimaryHandWeapon() > 0
                || getReloadableAmmoCountForSecondaryHandWeapon() > 0;
        return (canReload);
    }
     ...
     * 
     * @param bothPrimaryAndSecondary
     *            if <code>true</code> unselects the weapons in both hands,
     *            otherwise only unselects the secondary hand weapon
     * @return
     */
    protected boolean unselectWeaponInUse(final boolean bothPrimaryAndSecondary) {
        ...
        if (weaponInUse != null) {
            ...
            if (primaryHandWeaponNode != null) {
                ...
                // FIXME maybe hiding it would be enough, detaching it should be
                // done only when the player really drops it
            ...
            if (bothPrimaryAndSecondary) {// drops the secondary hand weapon
            ...
            if (success) {// updates this flag if no weapon is in use
                if ((primaryHandWeaponNode == null || !cameraNode.getChildren().contains(primaryHandWeaponNode))
                        && (secondaryHandWeaponNode == null
                                || !cameraNode.getChildren().contains(secondaryHandWeaponNode))) {
                    weaponInUse = null;
                    dualWeaponUseEnabled = false;
                } else {// this flag is set to true if the secondary hand weapon
                        // is still in use
                    dualWeaponUseEnabled = secondaryHandWeaponNode != null
                            && cameraNode.getChildren().contains(secondaryHandWeaponNode);
                }
            }
        }
    ...
    public boolean selectWeapon(final int index, final boolean dualWeaponUseWished) {
        ...
        /**
         * checks if: - the index is valid (i.e in [0;weaponCount[) - the weapon
         * is available in the primary hand - the weapon is available in the
         * secondary hand if the player wants to use one weapon per hand - the
         * player does not want to use one weapon per hand
         */
        final boolean success = chosenWeapon != null && primaryHandWeaponContainer.isAvailable(chosenWeapon)
                && (!dualWeaponUseWished || secondaryHandWeaponContainer.isAvailable(chosenWeapon));
        if (success) {
            ...
            if (oldWeaponIDInUse != chosenWeapon || oldDualWeaponUse != dualWeaponUseWished) {
                Node newWeapon;
                if (oldWeaponIDInUse != chosenWeapon) {// if at least one weapon
                                                       // was used previously
                    if (oldWeaponIDInUse != null) {// unselects them both
                        unselectWeaponInUse(true);
                    }
                    ...
                    if (dualWeaponUseWished) {// adds the left hand weapon
                ...
                {// FIXME dirty kludge, forces the ordinate of the right weapon
                    ...
                    if (dualWeaponUseWished) {// adds the left hand weapon
                        ...
                        initializeWeaponLocalTransform(newWeapon, false);
                        cameraNode.attachChild(newWeapon);
                    } else if (oldWeaponIDInUse != null) {// unselects the
                                                          // secondary hand
                                                          // weapon
            ...
            // updates these variables when the intermediary operations have
            // been performed
    ...
    protected boolean selectWeapon(final boolean next) {
        ...
        Entry<Integer, Boolean> selectableWeaponIndexAndDualHandEnabledFlag = getSelectableWeaponIndexAndDualHandEnabledFlag(
                next);
    ...
    /**
     * Returns the index of the weapon that can be selected if any and a flag
     * indicating whether it must be used in both hands, otherwise null
     * 
     * @param next
     *            flag indicating whether the returned weapon is the next one
     * @return the index of the weapon that can be selected if any and a flag
     *         indicating whether it must be used in both hands, otherwise null
     */
    public Entry<Integer, Boolean> getSelectableWeaponIndexAndDualHandEnabledFlag(final boolean next) {
        ...
        if (weaponCount >= 1) {
            ...
            // if the player chooses the next weapon, we have to increase the
            // weapon index, otherwise we have to decrease it
            if (next)
            ...
            if (weaponInUse != null) {
                ...
                if (next == dualWeaponUseEnabled)
                    ...
                    // tries to use the same weapon, only changes the number of
                    // weapons in use
                    firstInitialFactor = 0;
                final int weaponInUseId = weaponFactory.getIntIdentifier(weaponInUse);
                firstWeaponIndex = ((weaponInUseId + weaponCount) + (weaponIndexMultiplier * firstInitialFactor))
                        % weaponCount;
            } else
            ...
            if (weaponInUse != null)
                ...
                // if the player doesn't use any weapon yet
                // tries to use 2 identical weapons only if he wants the
                // previous one
                firstDualWeaponUseEnabledTested = !next;
            // there are 2 iterations per weapon (one for single handed, one for
            // dual handed)
            int iterationIndex = firstWeaponIndex * 2;
            // if we start with dual handed weapons, increases the iteration
            // index
            ...
            while (iterationCount < maxIterationCount && result == null) {
                ...
                /**
                 * checks if: - the index is valid (i.e in [0;weaponCount[) -
                 * the weapon is available in the primary hand - the player does
                 * not want to use one weapon per hand or the weapon is
                 * available in the secondary hand
                 */
                if (chosenWeapon != null && primaryHandWeaponContainer.isAvailable(chosenWeapon)
                        && (!dualWeaponUseEnabledTested || secondaryHandWeaponContainer.isAvailable(chosenWeapon)))
                    result = new AbstractMap.SimpleEntry<>(Integer.valueOf(currentWeaponIndex),
                            Boolean.valueOf(dualWeaponUseEnabledTested));
                else {// prepares the next iteration
    ...
    protected void initializeWeaponLocalTransform(Node newWeapon, boolean localizedInThePrimaryHand) {
        ...
        correctWeaponRotation.fromAngles(0, Math.PI, 0)
                .multiplyLocal(((WeaponUserData) newWeapon.getUserData()).getRotation());
