...
/**
 * Scenegraph state that modifies the parameters of the camera at runtime,
 * especially when entering and exiting this state. It sets them when entering
...
public class ScenegraphStateWithCustomCameraParameters extends ScenegraphState {
    ...
    public ScenegraphStateWithCustomCameraParameters(final SoundManager soundManager, final LogicalLayer logicalLayer,
            final Node root, final Camera cam) {
    ...
    public void setEnabled(final boolean enabled) {
        ...
        if (wasEnabled != isEnabled()) {
            ...
            if (isEnabled()) {
                ...
                cam.setFrustumPerspective(cam.getFovY(), (float) cam.getWidth() / (float) cam.getHeight(),
                        currentFrustumNear, currentFrustumFar);
                ...
                cam.setFrustumPerspective(cam.getFovY(), (float) cam.getWidth() / (float) cam.getHeight(),
                        previousFrustumNear, previousFrustumFar);
