...
bl_info = {
    "name": "Import .md2",
    "description": "Import in Quake2 file format (.md2)",
    "author": "Bob Holcomb, Julien Gouesse",
    "version": (1, 0),
    "blender": (2, 6, 3),
    "api": 46461,
    "location": "File > Import > md2",
...
class md2_obj(object):
	...
	def load (self, file):
		...
		for i in range(0,self.num_skins):
		...
		for i in range(0,self.num_tex_coords):
...
def load_textures(md2, texture_filename):
	...
	if int(md2.num_skins) > 0:
		for i in range(0,md2.num_skins):
			#md2.skins[i].dump()
...
def animate_md2(md2, mesh):
	...
	for i in range(0, md2.num_frames):
		frame = md2.frames[i]
		#update the vertices
		for j in range(md2.num_vertices):
...
def load_md2(md2_filename, texture_filename):
	...
	######### Creates a new mesh
	#mesh = Mesh.New()
	me = bpy.data.meshes.new("fucking_dataname")
	#me = bpy.data.meshes.new(dataname.decode('utf-8', "replace"))
	mesh = bpy.data.objects.new("Mesh",me)
	...
	#FIXME use me.vertices
	#mesh.verts.extend( [tmp_get_vertex(i) for i in range(0,md2.num_vertices)] )
	#del tmp_get_vertex
	...
	uv_list = [mathutils.Vector((co.u/w, 1-(co.v/h))) for co in md2.tex_coords]
	del w, h
...
class Import_MD2(bpy.types.Operator, ImportHelper):
	...
	def execute(self, context):
		...
		#TODO use the model filename and the texture filename (or -1 if none)
		return load_md2("/home/gouessej/Documents/programmation/java/artworks/models/md2/characters/male/tris.md2","")
	
	def draw(self, context):
		layout = self.layout
	#TODO use the new API to port the previous GUI
		return
...
def menuCB(self, context):
	self.layout.operator(Import_MD2.bl_idname, text="Import MD2...")
