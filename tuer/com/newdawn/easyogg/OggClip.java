/*This program is free software; you can redistribute it and/or
  ...
  MA 02111-1307, USA.
*/
//I am sorry, I cannot add an unlicensed source code file into my project
package com.newdawn.easyogg;
 ...
 * JOrbis.
 * 
 * @author kevin james glass
 * modified my Julien Gouesse to fit into the need of the project TUER
 */
public class OggClip {
	...
	private boolean balanceSupported=false;
	private boolean gainSupported=false;
	private Mixer mixer;
	...
	public OggClip(String ref) throws IOException{
	    try{init(Thread.currentThread().getContextClassLoader().getResourceAsStream(ref));} 
		catch (IOException e) 
	...
	public void play(){
		...
		player=new SingleSoundOneShotPlayer(this);
		player.setDaemon(true);
		player.start();
	}
	
	private static final class SingleSoundOneShotPlayer extends Thread{
        
	    private OggClip clip;
	    
	    private SingleSoundOneShotPlayer(OggClip clip){
	        super(SingleSoundOneShotPlayer.class.getName());
            this.clip=clip;
        }
	    
	    @Override
	    public final void run(){
	        try{clip.playStream(Thread.currentThread());} 
            catch(InternalException e)
            {e.printStackTrace();}              
            try{clip.bitStream.reset();} 
            catch(IOException e)
            {e.printStackTrace();}
        }
	}
	
	private static final class SingleSoundLoopPlayer extends Thread{
        
        private OggClip clip;
        
        private boolean stopped;
        
        private SingleSoundLoopPlayer(OggClip clip){
            super(SingleSoundLoopPlayer.class.getName());
            this.clip=clip;
            this.stopped=false;
        }
        ...
        @Override
        public final void run(){           
            while(this==Thread.currentThread()&&!stopped)
                {try{clip.playStream(Thread.currentThread());} 
                 catch(InternalException e)
                 {e.printStackTrace();
                  clip.player=null;
                  //the thread becomes useless, stop it
                  stopped=true;
                 } 
                 try{clip.bitStream.reset();} 
                 catch(IOException e)
	...
	public void loop(){
		...
		player=new SingleSoundLoopPlayer(this);
		player.setDaemon(true);
		player.start();
	}
	 ...
	 * but optimized to use a mixer better fitted to play
	 * multiple clips simultaneously
	 */
	private void initJavaSound(int channels, int rate) {
		try{AudioFormat audioFormat = new AudioFormat(rate, 16,
			...
			try{outputLine = (SourceDataLine) mixer.getLine(info);
